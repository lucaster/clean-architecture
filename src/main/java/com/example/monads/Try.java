...
/**
 * Monadic Try type.
 * Represents a result type that could have succeeded with type T or failed with
 * a Throwable.
 * Originally was Exception but due to seeing issues with eg play with checked
 * Throwable,
...
public abstract class Try<T> {
     ...
     * You need to be specific about the new type if changing type.
     *
     * Try.ofFailable(() -&gt; "1").&lt;Integer&gt;flatMap((x) -&gt;
     * Try.ofFailable(() -&gt; Integer.valueOf(x)))
     * returns Integer(1)
     *
     * @param f   function to apply to successful value.
     * @param <U> new type (optional)
    ...
    /**
     * Gets the value T on Success or throws the cause of the failure wrapped into a
     * RuntimeException
     *
     * @return T
    ...
    /**
     * If a Try is a Success and the predicate holds true, the Success is passed
     * further.
     * Otherwise (Failure or predicate doesn't hold), pass Failure.
     *
     * @param pred predicate applied to the value held by Try
     * @return For Success, the same success if predicate holds true, otherwise
     *         Failure
     */
